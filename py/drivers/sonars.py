...
class SonarsFilter:
    ...
    def add_measures(self, measures):
        ...
        for k in range(n):
            self.filters[k].add_measure(measures[k])

    def median_filter(self):
        return [flt.median_filter() for flt in self.filters]

    def centered_mean(self):
        return [flt.centered_mean() for flt in self.filters]
...
class Filter:
    ...
    def centered_mean(self):
        ...
        if len(self.buffer) < self.memorySize:
            ...
            return np.mean(np.sort(self.buffer)[k:k+self.meanArea])
