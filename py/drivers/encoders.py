...
class EncodersIO():
    ...
    def battery_voltage(self):
        # time.sleep(0.001)
        v = self.__read(offs=6)
        # print ("bat_lvl",v)
        v_mes = 5.0*v/1024.0
        v_bat = v_mes * 430.0/100.0
        # print (v,v_mes,v_bat)
        return v_bat

    def read_encoders_both_byte_test(self):
        rl = self.__read_byte(0x00)  # right low bit
        rh = self.__read_byte(0x01)  # right high bit
        ll = self.__read_byte(0x02)  # left low bit
        lh = self.__read_byte(0x03)  # left high bit
    ...
    def read_encoders(self):
        ...
        self.enc_left = self.__read(offs=0)
        self.enc_right = self.__read(offs=2)
